...
# Description: Automated end-to-end pipeline to compile Prism Yandex Mail Backup Tool Tool,
...
param (
    ...
    [switch]$AppImageOnly      # Pass -AppImageOnly to stop after creating Prism-Yandex-Mail-Backup-Tool.exe
)
...
$AppFolder = Join-Path $AppImageDir "Prism-Yandex-Mail-Backup-Tool"
...
Write-Host "  Prism Yandex Mail Backup Tool Tool - Full EXE & Installer Pipeline" -ForegroundColor Cyan
...
$JpackageArgs = @(
    "--type", "app-image",
    "--name", "Prism-Yandex-Mail-Backup-Tool",
...
$ExePath = Join-Path $AppFolder "Prism-Yandex-Mail-Backup-Tool.exe"
if (-not (Test-Path $ExePath)) {
    Write-Error "Prism-Yandex-Mail-Backup-Tool.exe was not created at expected location: $ExePath"
}
...
& $AiPath /edit $AipFile /SetProperty ProductName="Prism Yandex Mail Backup Tool Tool"
...
& $AiPath /edit $AipFile /NewShortcut -name "Prism Yandex Mail Backup Tool Tool" -dir DesktopFolder -target "[APPDIR]Prism-Yandex-Mail-Backup-Tool\Prism-Yandex-Mail-Backup-Tool.exe" -icon "$LogoIcon"
& $AiPath /edit $AipFile /NewShortcut -name "Prism Yandex Mail Backup Tool Tool" -dir SHORTCUTDIR -target "[APPDIR]Prism-Yandex-Mail-Backup-Tool\Prism-Yandex-Mail-Backup-Tool.exe" -icon "$LogoIcon"
...
& $AiPath /edit $AipFile /SetPackageName "Prism-Yandex-Mail-Backup-Tool-Setup.exe" -buildname DefaultBuild
...
# 10. Sync to Drive
$driveJarDir = "I:\My Drive\Akas\Jars\Yandex Mail Backup Tool"
if (-not (Test-Path $driveJarDir)) { New-Item -ItemType Directory -Path $driveJarDir -Force | Out-Null }
Copy-Item $ProtectedJar.FullName (Join-Path $driveJarDir "Prism-Yandex-Mail-Backup-Tool.jar") -Force
...
Copy-Item $AipFile (Join-Path $destAdvance "Prism-Yandex-Mail-Backup-Tool.aip") -Force
...
Write-Host "     -> Prism-Yandex-Mail-Backup-Tool-Setup.exe" -ForegroundColor White
Write-Host "  3. Standalone Protected JAR: $driveJarDir" -ForegroundColor White
Write-Host "     -> Prism-Yandex-Mail-Backup-Tool.jar" -ForegroundColor White
Write-Host "========================================================" -ForegroundColor Green
