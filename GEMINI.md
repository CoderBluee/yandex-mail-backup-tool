# Prism Yandex Mail Backup Tool — JavaFX Desktop Application (Modular Template)
...
- **Package fat JAR & Obfuscate**:
  ...
  - `target/yandex-mail-backup-tool-1.0-SNAPSHOT-win-<yyyyMMdd-HHmm>.jar` (~110 MB)
  - `target/yandex-mail-backup-tool-1.0-SNAPSHOT-win-<yyyyMMdd-HHmm>-protected.jar` (~103 MB)
- **Dependencies**:
...
|---|---|---|
| **Single-File Setup Installer** | Final installer EXE for end-users (Advanced Installer `ExeInside`, shortcuts, icons, splash) | `I:\My Drive\Akas\EXE\Prism-Yandex-Mail-Backup-Tool-Setup.exe` (~135.4 MB) |
| **Advanced Installer Project** | `.aip` file source for setup configurations | `I:\My Drive\Akas\Package Maker\Advance Package\Prism-Yandex-Mail-Backup-Tool.aip` |
| **Packaging Automation Script** | Reusable pipeline script synced to Drive | `I:\My Drive\Akas\Package Maker\Advance Package\package_exe_yandex_mail_backup_tool.ps1` |
| **Standalone Protected JAR** | Deployed copy of latest obfuscated Java 21 JAR | `I:\My Drive\Akas\Jars\IMAP Backup\Prism-Yandex-Mail-Backup-Tool.jar` (~103.65 MB) |
| **Local Standalone App-Image** | Portable runtime folder with launcher for local developer testing | `target/app-image/Prism-Yandex-Mail-Backup-Tool/Prism-Yandex-Mail-Backup-Tool.exe` (~583 KB launcher + bundled JRE) |

### Understanding the 2 Types of EXEs
1. **Portable Launcher EXE (`target/app-image/.../Prism-Yandex-Mail-Backup-Tool.exe` - 583 KB)**:
...
2. **Single-File Setup Installer EXE (`I:\My Drive\Akas\EXE\Prism-Yandex-Mail-Backup-Tool-Setup.exe` - 135.4 MB)**:
...
- **Report file**: `~/Documents/Prism Yandex Mail Backup Tool/testing_report.txt` (appended across runs; one session per run separated by `═══` banners).
